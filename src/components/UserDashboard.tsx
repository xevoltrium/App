...
export function UserDashboard({ 
  ...
  const createPlan = async (requestOverride?: string) => {
    setGenerating(true);
    const finalRequest = requestOverride || specialRequest || "Fokus auf allgemeine Fitness & Ganzkörpertraining";
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Fehler", description: "Plan konnte nicht generiert werden. Bitte versuche es erneut." });
    } finally {
      setGenerating(false);
    }
  };
            ...
            <section className="bg-primary/5 rounded-3xl p-8 border border-primary/10 flex flex-col md:flex-row gap-8 items-center justify-between overflow-hidden relative">
              ...
              <div className="space-y-4 max-w-lg w-full">
                <div className="flex items-center gap-2 text-primary font-bold">
                  ...
                  <span>KI-Trainer aktiv</span>
                </div>
                <h2 className="text-3xl font-headline font-bold leading-tight">Was möchtest du heute erreichen?</h2>
                
                <div className="space-y-2">
                  <Label htmlFor="special-req" className="text-xs text-muted-foreground">Spezialisierungswunsch (z.B. "Fokus auf Rücken", "30 Min HIIT", etc.)</Label>
                  <div className="flex gap-2">
                    <Input 
                      id="special-req"
                      placeholder="Was ist dein Ziel für diesen Plan?" 
                      className="bg-white rounded-full h-12 px-6 shadow-sm border-primary/20"
                    ...
                    <Button 
                      onClick={() => createPlan()} 
                      disabled={generating}
                      className="rounded-full h-12 w-12 shrink-0 bg-primary shadow-lg hover:shadow-xl transition-all"
                    >
        ...
        {activeTab === 'chat' && (
          <section className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">Fragen zu Übungen, Ernährung oder deinem Plan?</p>
            </div>
            <div className="bg-white border rounded-2xl shadow-sm flex flex-col h-[550px] overflow-hidden">
              <ScrollArea className="flex-1 p-6">
                <div className="space-y-4">
                  <div className="flex justify-start">
                    <div className="max-w-[80%] p-4 rounded-2xl bg-muted rounded-tl-none text-sm">
                      Hallo {user.name}! Ich bin dein persönlicher KI-Trainer. Wie kann ich dir heute bei deinem Ziel "{user.fitnessGoal}" helfen?
                    </div>
                  </div>
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-none' : 'bg-muted rounded-tl-none'}`}>
                        {msg.content}
                      </div>
                    </div>
                  ))}
              ...
              <div className="p-4 border-t flex gap-2 bg-muted/5">
                <Input 
                  ...
                  className="bg-white"
                />
                <Button onClick={handleSendChat} disabled={sendingChat || !chatInput.trim()} className="bg-primary text-primary-foreground">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  ...
                  <Badge className="bg-accent text-accent-foreground">{user.bmiLevel}</Badge>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label>Besondere Wünsche (für KI-Planer)</Label>
                    <Textarea 
                      value={user.specialPreferences || ''} 
                      onChange={e => {
                        updateUser({ specialPreferences: e.target.value });
                        setSpecialRequest(e.target.value);
                      }}
                      placeholder="z.B. Fokus auf Kraft, keine Sprünge..."
                    />
                  </div>
                </CardContent>
              </Card>
                  ...
                  >
                    {['underweight', 'normal', 'overweight', 'obese'].map(l => (
                      <div key={l} className="flex items-center space-x-2">
                        ...
                        <Label className="capitalize" htmlFor={`prof-bmi-${l}`}>
